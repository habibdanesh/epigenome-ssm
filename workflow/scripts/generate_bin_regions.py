...
with open(n_bins_file, 'w') as n_bins_f:
        ...
        with open(chrom_regions_file, 'w') as chrom_regions_f:
            for line in in_regions[region_counter:]:
                ...
                while bin_start < region_end:
                    ...
                    bin_counter += 1 # counts the total number of bins for each chrom
                    region_n_bins += 1 # counts the total number of bins for each region
                    # move
                    bin_start = bin_end
                    bin_end += bin_size
                print("{}\t{}\t{}\t{}".format(chrom, region_start, region_end, region_n_bins), file=n_bins_f)
