"""
Get feature values within enhancer per bin
...
Output:
    - Feature values within enhancer per bin
"""
...
region_col_names = ['epigenome', 'chrom', 'base', 'final', 'activity value', 'active']
region_df = None
annotation_df = None
NUM_BINS = None
enhancer_region_file = None
TEMP_DIR = None
...
def main(argvs):
    ...
    for region in region_df.itertuples(index=False, name=None):
        feature_df.append(get_enhancer_bin_features(region))
    ...
    region_df.to_csv(os.path.join(TEMP_DIR, \
        epigenome+'-'+chromosome+'_enhancer_bin_features-' + 'k'+str(k) + '.csv'), index=False)


def read_enhancer_info():
    global region_df
    global epigenome
    global chromosome
    global region_col_names
    global enhancer_region_file
    col_dtype = {'chrom':np.string_, 'base':np.uint32, 'final':np.uint32, 'activity value':np.float64, \
        'active':np.bool_}
    region_df = pd.read_csv(enhancer_region_file, \
        names=region_col_names[1:5], header=None, delim_whitespace=True, dtype=col_dtype)
    enhancer_std = sorted(region_df['activity value'], reverse=True)[int(len(region_df['activity value']) / 3)]
    region_df = region_df[region_df['chrom'] == chromosome]
    region_df['active'] = region_df['activity value'] >= enhancer_std
...
def get_enhancer_bin_features(region):
    '''
    region: ['chrom', 'base', 'final', 'activity value', 'active']
    '''
    global annotation_df
    global k
    global NUM_BINS
